...
trait SigPowMainIOApp {
    ...
    val runMain = for {
        _ <- printlnIO("""| Please enter one of the following commands:
        ...
        _ <- cmd.flatMap {
          ...
          case "unlock" =>
            printlnIO("""| So, you know about a work-locked utxo that you would like to
          ...
          case _ => printlnIO("Invalid command. Bye!")
        }
    } yield ()

    val createWorkLockedOutput = for {
        _ <- printlnIO("""| This program will create an output (and an address
        ...
        _ <- printlnIO(s"Great! Using sha256($seed) == ${h_seed.toHex} as seed")
        ...
        _ <- printlnIO(s"this gives a range of $block_range possible nLocktimes")
        num_possible_sigs <- IO(log(block_range.toDouble,2).ceil.toInt)
        _ <- printlnIO(s"we will need $num_possible_sigs signatures to encode a number in this range")
        _ <- printlnIO("note: with our current encoding mechanism, using more than approximately 14 signatures may create a bitcoin script with too many non-push operatiosn (an invalid script).")
        ...
        _ <- printlnIO("""| Now we need to calibrate your work-lock. A signature
         ...
         _ <- printlnIO("""| Ok, here are the private keys, public keys, and max signature lengths
                            | needed in order to reduce the locktime for a spending transaction
                            | down to zero (no locktime). Good luck!
                            |""".stripMargin)
         _ <- printlnIO("(index_i, private_key_i, pub_key_i, max_siglength_bytes_i)")
         _ <- priv_keys.zip(sig_sizes).zipWithIndex
                .traverse{ case ((k,s),i) => printlnIO(s"($i, ${k.value.toHex}, ${k.publicKey}, $s)")}
         ...
         _ <- printlnIO(s"redeem script (in hex): ${redeemScript.toHex} \n")
         pubKeyScript_p2wsh <- SigPow.pubKeyScript[IO](redeemScript)
         _ <- printlnIO(s"p2wsh pubkey script (in hex, ${pubKeyScript_p2wsh.size} bytes): ${pubKeyScript_p2wsh.toHex}")
         _ <- printlnIO(s"testnet p2wsh segwit address: ${Bech32.encodeWitnessAddress("tb",0,pubKeyScript_p2wsh.drop(2))}")
         _ <- printlnIO(s"mainnet p2wsh segwit address: ${Bech32.encodeWitnessAddress("bc",0,pubKeyScript_p2wsh.drop(2))}")
         ...
         fundingTx <- if(sendFakeFunds) {
              SigPow.fakeP2WSHFundingTx[IO](1000000L,redeemScript).flatTap(t => 
                printlnIO(s"funding transaction txid: ${t.txid}") >>
                printlnIO(s"the work-locked sats are at output index 0 wrapped in P2WSH") >> 
                printlnIO(s"funding transaction (hex): $t") >>
                printlnIO("1,000,000 sats now (fake) work-locked!"))
          } else { 
         ...
         //_ <- printlnIO(s"here are $num_sigs fake sig_i:pubkey_i pairs (useful for debugging with btcdeb):")
         //_ <- printlnIO(fakeSigs.map{ case (_, sig_i, pub_i) => s"${sig_i.toHex}:$pub_i"}.mkString(","))
         //_ <- printlnIO(s"here is a fake stack of signatures (useful for debugging with btcdeb):")
         //_ <- printlnIO(fakeSigs.map{ case (_, sig_i, _) => sig_i.toHex }.reverse.mkString(" "))
         _ <- printlnIO("End locking procedure.")
    ...
    def spendWorkLockedOutput(fundingTx: Transaction): IO[Transaction] = for {
      ...
      _ <- printlnIO(s"got txid: ${fundingTxid.toHex}")
      ...
      _ <- printlnIO(s"output index $fundingOutputIndex has $worklockedSatsAmt sats work-locked!")
      ...
      redeemScript <- SigPow.redeemScript[IO](priv_keys.map(_.publicKey).zip(sig_sizes),minLocktime)
      _ <- printlnIO(s"reconstructed redeem script (in hex): ${redeemScript.toHex} \n")
      pubKeyScript_p2wsh <- SigPow.pubKeyScript[IO](redeemScript)
      _ <- printlnIO(s"reconstructed the p2wsh pubkey script (in hex): ${pubKeyScript_p2wsh.toHex}")
      _ <- printlnIO(s"-------- now lets start mining ------")
      _ <- printlnIO("""| You now need to specify a "target" locktime which you want
      ...
      _ <- printlnIO(s"(mainnet) address: ${Bech32.encodeWitnessAddress("bc",0,spendToPubKeyScriptBytes.drop(2))}")
      _ <- printlnIO(s"OR (testnet) address: ${Bech32.encodeWitnessAddress("tb",0,spendToPubKeyScriptBytes.drop(2))}")
      ...
      _ <- printlnIO(s"unsigned spending transaction (hex): $unsignedSpendingTx")
      ...
      _ <- priv_keys.map(_.publicKey.value.toHex).zip(signatures).traverse{ case (pub,sig) => printlnIO(s"$pub --sig--> ${sig.toHex}")}
      witness <- SigPow.witness[IO](redeemScript,signatures)
      signedSpendingTx = unsignedSpendingTx.updateWitness(0,witness)
      _ <- printlnIO(s"SIGNED spending transaction (hex): $signedSpendingTx")
      _ <- printlnIO("Done.")
      _ <- IO(Transaction.correctlySpends(signedSpendingTx,Seq(fundingTx),ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS)).onError(e => printlnIO(e.getMessage()))
    ...
    def printlnIO(msg: String): IO[Unit] = IO.println(msg)
    def printIO(msg: String): IO[Unit] = IO.print(msg)
    def prompt(msg: String): IO[String] = IO.print(msg + " ") >> IO.readLine.flatTap(m => printlnIO(s"you entered: $m"))
